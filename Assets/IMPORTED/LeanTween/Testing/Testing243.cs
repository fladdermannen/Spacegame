...
//		LeanTween.moveLocal(cube1, cube1.transform.localPosition+new Vector3(0f,1f,0f),1f).setEaseShake();
		...
		LeanTween.sequence().append(LeanTween.value (lightt.gameObject, (float f) =>	lightt.range = f, 8, 12, .1f)).append (LeanTween.value (lightt.gameObject, (float f) =>	lightt.range = f, 12, 8, .2f));
	} 
